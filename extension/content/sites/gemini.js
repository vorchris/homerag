(function () {
  ...
  function getVisiblePromptEl() {
    // Gemini uses a rich-textarea with a Quill editor inside
    const candidates = [
      ...document.querySelectorAll('rich-textarea div[contenteditable="true"]'),
  ...
  function findComposerContainer() {
    ...
    for (let i = 0; i < 10; i++) {
      el = el.parentElement
      if (!el) break
      if (el.tagName === 'FORM' || el.localName === 'rich-textarea') return el
    }
    return sendBtn.closest('form') ?? sendBtn.parentElement ?? null
  }
