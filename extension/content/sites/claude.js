(function () {
  ...
  function getSendButton() {
    return (
      ...
      // Claude uses aria-label with the send arrow
      [...document.querySelectorAll('button')].find(b => {
        const a = (b.getAttribute('aria-label') || '').toLowerCase()
        return a.includes('send') || a.includes('senden')
      }) ||
      // Last resort: find any button with an SVG near the input
      [...document.querySelectorAll('button[type="button"]')].findLast(b => b.querySelector('svg'))
    )
  }
  ...
  function findComposerContainer() {
    ...
    // Find closest common ancestor of input + send button (max 8 levels)
    if (sendBtn) {
      let el = promptEl.parentElement
      for (let i = 0; i < 8; i++) {
        if (!el || el === document.body) break
        if (el.contains(sendBtn)) {
          console.log('[HomeRAG claude] composer via send-button ancestor:', el)
          return el
        }
        el = el.parentElement
      }
    ...
    // Fallback: first ancestor with constrained height (input bar, not full page)
    let el = promptEl.parentElement
    while (el && el !== document.body) {
      const rect = el.getBoundingClientRect()
      if (rect.width > window.innerWidth * 0.3 && rect.height > 40 && rect.height < window.innerHeight * 0.4) {
        console.log('[HomeRAG claude] composer via height fallback:', el)
        return el
      }
      el = el.parentElement
    }
  ...
  document.addEventListener('input', () => {
    ...
    debounceTimer = setTimeout(async () => {
      ...
      console.log('[HomeRAG claude] query:', query, '→ chunks:', chunks.length)
      suggestions = chunks
  ...
  setTimeout(() => {
    const promptEl = getVisiblePromptEl()
    const composer = findComposerContainer()
    console.log('[HomeRAG claude] promptEl:', promptEl)
    console.log('[HomeRAG claude] composer:', composer)
    console.log('[HomeRAG claude] composer.parentElement:', composer?.parentElement)
  }, 2000)
